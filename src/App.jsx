...
const STATUS_MAP = {
  da_scrivere: { label: 'Da Scrivere', color: 'bg-slate-100 text-slate-600' },
  in_corso:    { label: 'In Corso',    color: 'bg-amber-100 text-amber-700' },
  pubblicato:  { label: 'Pubblicato',  color: 'bg-emerald-100 text-emerald-700' },
};
...
export default function App() {
  const [activeTab, setActiveTab] = useState('analisi'); // 'analisi' | 'piano'
  ...
  useEffect(() => {
    try {
      ...
      if (saved) {
        const { result: savedResult, news: savedNews, topic, savedIds: savedIdsArr, date } = JSON.parse(saved);
        setResult(savedResult);
        setNews(savedNews || []);
        setSelectedTopic(topic);
        setSavedIds(new Set(savedIdsArr || []));
  ...
  // Load piano editoriale when tab switches
  useEffect(() => {
    if (activeTab === 'piano') {
      loadPiano();
    }
  ...
  async function loadPiano() {
    ...
    const items = await fetchPianoEditoriale(60);
    setPiano(items);
  ...
  async function handleSaveOne(article) {
    try {
      ...
      const newSavedIds = new Set([...savedIds, article.title]);
      setSavedIds(newSavedIds);
      // Update localStorage too
      const saved = localStorage.getItem(LS_KEY);
      if (saved) {
        const data = JSON.parse(saved);
        data.savedIds = [...newSavedIds];
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC]">
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex gap-1 pb-0">
            {[
              { id: 'analisi', label: 'Analisi Trend', icon: Sparkles },
              { id: 'piano',   label: 'Piano Editoriale', icon: BookOpen },
            ].map(tab => (
              <button
                key={tab.id}
                onClick={() => setActiveTab(tab.id)}
                className={cn(
                  "flex items-center gap-2 px-5 py-3 text-sm font-bold transition-all border-b-2 cursor-pointer",
                  activeTab === tab.id
          ...
          <div className="space-y-8">
              ...
              <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
                ...
                {lastAnalysisDate && (
                  <div className="flex items-center gap-2 text-xs text-slate-400">
                    ...
                    Ultima analisi: {new Date(lastAnalysisDate).toLocaleString('it-IT', { dateStyle: 'short', timeStyle: 'short' })}
                    <button onClick={clearAnalysis} className="text-rose-400 hover:text-rose-600 ml-1 underline font-medium">
                      Cancella
                    </button>
                  </div>
                )}
            ...
            {status === 'error' && (
              <div className="bg-red-50 border border-red-100 rounded-2xl p-6 flex items-start gap-4">
                ...
                <div>
                  ...
                  <button onClick={handleAnalyze} className="mt-4 text-sm font-bold text-red-700 hover:text-red-900 underline">Riprova</button>
                </div>
              </div>
            )}
            ...
            {status === 'loading' && (
              <div className="card-premium text-center py-16">
                ...
                <p className="text-slate-600 font-medium text-lg">{phase}</p>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                  ...
                  <div className="lg:col-span-4 space-y-4">
                    <div className="flex items-center justify-between px-2">
                      ...
                      <span className="text-[10px] font-bold text-slate-400 bg-slate-100 px-2 py-0.5 rounded-full">{news.length} SORGENTI</span>
                    </div>
                    <div className="bg-white border border-slate-100 rounded-[28px] overflow-hidden shadow-sm h-[600px] overflow-y-auto">
                      <div className="divide-y divide-slate-50">
                        {news.map((n, i) => (
                          <a key={i} href={n.link} target="_blank" rel="noreferrer" className="p-5 block hover:bg-slate-50 group transition-colors">
                  ...
                  <div className="lg:col-span-8 space-y-6">
                    <div className="flex items-center justify-between px-2">
                      <h3 className="font-bold text-slate-900 flex items-center gap-2 uppercase tracking-tight text-sm">
                        <Lightbulb className="w-4 h-4 text-amber-500" />Suggerimenti Editoriali
                      </h3>
                      <span className="text-xs font-medium text-slate-500">
                        {savedIds.size} / {result.articles.length} salvati
                      </span>
                    </div>
                    <div className="grid grid-cols-1 gap-4">
                      {result.articles.map((article, i) => {
                        ...
                        return (
                          <div key={i} className="card-premium relative overflow-hidden">
                            <div className={cn(
                            ...
                            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-6">
                              <div className="space-y-4 flex-1">
                                <div className="flex flex-wrap items-center gap-3">
                                  <span className={cn("pill", article.urgenza === 'alta' ? 'pill-danger' : article.urgenza === 'media' ? 'pill-warning' : 'bg-slate-50 border-slate-200 text-slate-500')}>
                                    {article.urgenza === 'alta' ? 'Alta Urgenza' : article.urgenza === 'media' ? 'Media Urgenza' : 'Bassa Urgenza'}
                                  </span>
                                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{article.tipo}</span>
                                </div>
                                <div>
                                  <h4 className="text-xl font-bold text-slate-900 mb-2 leading-tight">{article.title}</h4>
                                  <p className="text-sm text-slate-500 italic mb-4 leading-relaxed line-clamp-2">{article.rationale}</p>
                                </div>
                                <div className="flex flex-wrap items-center gap-3 pt-2">
                                  ...
                                  <div className="flex items-center gap-1 text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                                    ...
                                    Traffico: <span className="text-slate-600 ml-1">{article.stima_traffico}</span>
                                  </div>
                                </div>
                              </div>
                              <div className="sm:shrink-0 flex sm:flex-col gap-2">
                                <button
                                  ...
                                  className={cn("btn-premium flex-1 sm:flex-none", isSaved ? "bg-emerald-50 text-emerald-600 border border-emerald-100" : "btn-indigo")}
                                >
                    ...
                    {result.trending_keywords.map((kw, i) => (
                      <div key={i} className="p-5 rounded-2xl border border-slate-100 bg-slate-50/50 hover:bg-white hover:border-indigo-100 transition-all hover:-translate-y-1">
                        <div className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 flex items-center justify-between">
                          {kw.volume_stimato} vol.<ChevronRight className="w-3 h-3 text-slate-300" />
                        </div>
                        <div className="font-bold text-slate-800 mb-3 break-words">{kw.keyword}</div>
                        <div className={cn("text-[9px] font-black uppercase px-2 py-0.5 rounded inline-block",
            ...
            {status === 'idle' && (
              <div className="text-center py-20 px-4">
                ...
                <p className="text-slate-500 max-w-lg mx-auto leading-relaxed text-lg">
                  Seleziona un settore per scoprire su cosa scrivere oggi. Analizziamo news, subreddit e query reali per darti suggerimenti ad alto traffico.
                </p>
              </div>
            )}
          </div>
          ...
          <div className="space-y-6">
            ...
            {pianoLoading ? (
              <div className="card-premium text-center py-16">
                ...
                <p className="text-slate-500">Caricamento piano editoriale...</p>
              </div>
            ) : piano.length === 0 ? (
              <div className="card-premium text-center py-16">
                <BookOpen className="w-12 h-12 text-slate-300 mx-auto mb-4" />
                <h3 className="font-bold text-slate-700 mb-2">Piano editoriale vuoto</h3>
                <p className="text-slate-400 text-sm">Aggiungi articoli dall'analisi trend per vederli qui.</p>
              </div>
            ) : (
                ...
                <div className="overflow-x-auto">
                  <table className="zebra-table">
                    ...
                    <tbody>
                      {piano.map((item) => (
                        <tr key={item.id}>
                          <td className="font-semibold text-slate-800 max-w-xs">
                          ...
                          <td>
                            <span className="text-xs font-bold uppercase tracking-widest text-slate-500">{item.cat || '—'}</span>
                          </td>
                          <td>
                            {item.diff && (
                              <span className={cn("text-[10px] font-black uppercase px-2 py-0.5 rounded", DIFF_MAP[item.diff] || 'bg-slate-100 text-slate-600')}>
                                {item.diff}
                              </span>
                            )}
                          </td>
                          <td>
                            {(() => {
                              const s = STATUS_MAP[item.status] || { label: item.status || '—', color: 'bg-slate-100 text-slate-600' };
                              return <span className={cn("text-[10px] font-black uppercase px-2 py-0.5 rounded", s.color)}>{s.label}</span>;
                            })()}
                          </td>
                          <td>
                            <span className="text-xs text-slate-400">{item.source === 'trend-scout' ? '🔭 Scout' : '✏️ Manuale'}</span>
                          </td>
                        </tr>
                      ))}
                    </tbody>
      ...
      <footer className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 border-t border-slate-100 text-center mt-8">
